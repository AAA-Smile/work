...
plt.axis('off')
plt.title('播放量Top10视频标签词云', fontsize=16)
plt.tight_layout()
plt.savefig('播放量Top10视频标签词云.png', dpi=150)
plt.show()

# ========== 7. 可选：保存词频表 ==========
pd.DataFrame(tag_counts.items(), columns=['标签', '频次']).to_csv('Top10标签频次.csv', index=False, encoding='utf-8-sig')
print("词云已保存为 播放量Top10视频标签词云.png，词频表已保存为 Top10标签频次.csv")